...
class _BillDetailPageState extends ConsumerState<BillDetailPage> {
  ...
  void _showBankUpdateConfirmDialog(
  ...
  void _showUnassignedDetailDialog(
    BuildContext context,
    List<BillItemEntity> unassignedItems,
  ) {
              ...
              child: Column(
                children: [
                  _buildMismatchRow(
                    'Tiền món (Subtotal):',
                    CurrencyFormatter.formatVND(
                      computedNetItemsTotal.toDouble(),
                    ),
                    isDark: isDark,
                  ),
                  ...
                  _buildMismatchRow(
                    'Phí dịch vụ:',
                    serviceCharge > 0
                        ? '+ ${CurrencyFormatter.formatVND(serviceCharge.toDouble())}'
                        : '0 đ',
                    isDark: isDark,
                  ),
                  const SizedBox(height: 4),
                  _buildMismatchRow(
                    'Thuế VAT:',
                    vat > 0
                        ? '+ ${CurrencyFormatter.formatVND(vat.toDouble())}'
                        : '0 đ',
                    isDark: isDark,
                  ),
                  const SizedBox(height: 4),
                  _buildMismatchRow(
                    'Giảm giá khác:',
                    generalDiscount > 0
                        ? '- ${CurrencyFormatter.formatVND(generalDiscount.toDouble())}'
                        : '0 đ',
                    isDark: isDark,
                  ),
                  ...
                  _buildMismatchRow(
                    '= Tổng tính toán:',
                    CurrencyFormatter.formatVND(computedTotal.toDouble()),
                    isBold: true,
                    isDark: isDark,
                  ),
                  const SizedBox(height: 4),
                  _buildMismatchRow(
                    'Tổng cộng trên bill:',
                    CurrencyFormatter.formatVND(billTotal.toDouble()),
                    isBold: true,
                    isDark: isDark,
                  ),
                  const Divider(height: 12),
  ...
  Widget build(BuildContext context) {
    ...
    final hasBankAccount =
        currentUser == null ||
        (currentUser.bankAccountNumber != null &&
            currentUser.bankAccountNumber!.trim().isNotEmpty);
    ...
    final isTotalMismatch =
        !hasNoItems && state.computedTotal != state.bill.total;
    ...
    final isCreditor =
        currentUserId.isEmpty ||
        isNewUnsavedBill ||
        bill.creditorMemberId.isEmpty ||
        bill.members.any(
          (m) =>
              m.userId == currentUserId && m.memberId == bill.creditorMemberId,
        );

    final isReadOnlyStatus =
        bill.status == 'finalized' || bill.status == 'voided';
    ...
    final canEdit =
        !isReadOnlyStatus &&
        (isCaptain || isCreditor || isNewUnsavedBill || bill.members.isEmpty);

    final displayDate = (bill.createdAt ?? bill.billDate ?? DateTime.now())
        .toLocal();
    ...
    return Scaffold(
        ...
        title: Row(
          ...
          children: [
            ...
            if (canEdit) ...[
              ...
              InkWell(
                onTap: () =>
                    _showEditMerchantDialog(context, bill.merchantName ?? ''),
            ...
            child: Row(
              children: [
                ...
                if (bill.photos.isNotEmpty) ...[
                  ...
                  InkWell(
                    onTap: () =>
                        ImageViewerDialog.show(context, photos: bill.photos),
                    borderRadius: BorderRadius.circular(10),
                    child: Container(
                      decoration: BoxDecoration(
                        border: Border.all(
                                      ...
                                      onTap: canEdit
                                          ? () {
                                              SelectEvenSplitMembersModal.show(
                                                context,
                                                members: bill.members,
                                                initialSelectedMemberIds: state
                                                    .activeEvenSplitMemberIds,
                                                creditorMemberId:
                                                    bill.creditorMemberId,
                                                onConfirm: (selectedIds) {
                                                  notifier.setEvenSplitMembers(
                                                    selectedIds,
                                                  );
                                                },
                                              );
                  ...
                  if (bill.items.isEmpty)
                    ...
                    for (int i = 0; i < bill.items.length; i++) ...[
                      BillItemCard(
                        ...
                        onToggleMember: (mId) => notifier
                            .toggleMemberAssignment(bill.items[i].id, mId),
                        onAssignAll: () =>
                            notifier.assignAllMembersToItem(bill.items[i].id),
                      ),
                    ],
      ...
      bottomNavigationBar: state.isLoading
          ...
          : BillStickyBottomBar(
              ...
              onUpdateBankAccount: () =>
                  _showBankUpdateConfirmDialog(context, notifier),
              onOpenUnassignedDetail: () {
                final unassignedItems = state.bill.items
                    .where((i) => i.assignments.isEmpty)
                    .toList();
              ...
              onOpenBreakdown: () async {
                final officialBreakdown = await notifier
                    .fetchOfficialBreakdown();
                if (context.mounted) {
                  final latestState = ref.read(
                    billDetailNotifierProvider(_initialBill),
                  );
                  await BillBreakdownBottomSheet.show(
                    context,
                    breakdown: officialBreakdown.isNotEmpty
                        ? officialBreakdown
                        : latestState.breakdown,
              ...
              onFinalize: () async {
                ...
                if (confirmed == true && context.mounted) {
                  ...
                  } else if (result.isVersionConflict && context.mounted) {
                    await _showVersionConflictDialog(context, notifier);
                  }
                }
              },
            ),
  ...
  /// Hiển thị dialog khi phát hiện xung đột dữ liệu phiên bản (VERSION_CONFLICT / 409)
  Future<void> _showVersionConflictDialog(
    ...
    if (shouldReload == true && context.mounted) {
      final reloaded = await notifier.reloadBill();
      if (reloaded && context.mounted) {
        showSuccessSnackBar(context, 'Đã tải dữ liệu mới nhất của hoá đơn!');
      }
    }
  }
  ...
  Future<void> _showVoidBillDialog(
    BuildContext context,
    BillDetailNotifier notifier,
  ) async {
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'Nhập lý do huỷ (VD: Sai số tiền, tính nhầm món...)',
                    hintStyle: GoogleFonts.plusJakartaSans(
                      fontSize: 13,
                      color: isDark
                          ? const Color(0xFF64748B)
    ...
    if (confirmed == true && context.mounted) {
      final success = await notifier.voidBill(
