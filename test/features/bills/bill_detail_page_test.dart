...
void main() {
  ...
  final sampleBill = BillDetailEntity(
    ...
    items: [
      const BillItemEntity(
        ...
        finalPrice: 300000,
        assignments: [
          BillItemAssignmentEntity(memberId: 'm-1', displayName: 'Tin', weight: 0.5),
          BillItemAssignmentEntity(memberId: 'm-2', displayName: 'Nam', weight: 0.5),
        ],
      ),
      const BillItemEntity(
        id: 'item-2',
        ...
        assignments: [
          BillItemAssignmentEntity(memberId: 'm-2', displayName: 'Nam', weight: 0.5),
          BillItemAssignmentEntity(memberId: 'm-3', displayName: 'Linh', weight: 0.5),
        ],
      ),
    ],
  );
  ...
  group('BillDetailNotifier Unit Tests', () {
    ...
    test('toggles member assignment and updates weight', () {
      ...
      final updatedItem1 = notifier.state.bill.items.firstWhere((i) => i.id == 'item-1');
      ...
      notifier.toggleMemberAssignment('item-1', 'm-3');
      final revertedItem1 = notifier.state.bill.items.firstWhere((i) => i.id == 'item-1');
      expect(revertedItem1.assignments.length, 2);
      expect(revertedItem1.assignments.any((a) => a.memberId == 'm-3'), isFalse);
    });

    test('switches split mode to even with custom members and preserves unified assignments when switching back to item_ratio', () {
      final notifier = BillDetailNotifier(mockRepository, sampleBill);
      ...
      expect(notifier.state.evenPerPersonAmount, 233333); // 700k / 3 = 233,333 đ
      ...
      expect(notifier.state.evenPerPersonAmount, 350000); // 700k / 2 = 350,000 đ
    ...
    test('updating quantity to 2 on one item modifies only that item without corrupting others', () {
      final notifier = BillDetailNotifier(mockRepository, sampleBill);
    ...
    test('updates item, taxes, and detects delta mismatch with independent reported total', () {
      final notifier = BillDetailNotifier(mockRepository, sampleBill);
    ...
    test('adding a new item recalculates computed total and shows mismatch against reported total', () {
      final notifier = BillDetailNotifier(mockRepository, sampleBill);
      ...
      expect(notifier.state.bill.total, 700000); // Reported total is preserved!
      expect(notifier.state.deltaTotal, 50000); // Mismatch detected!
    });

    test('deleting an item recalculates totals and shows mismatch against reported total', () {
      final notifier = BillDetailNotifier(mockRepository, sampleBill);
      ...
      expect(notifier.state.bill.total, 700000); // Reported total is preserved!
      expect(notifier.state.deltaTotal, -300000); // Missing 300k compared to reported total!

    });

    test('calculateBreakdown and fetchOfficialBreakdown calls calculate API and updates state breakdown', () async {
      when(() => mockRepository.calculateBreakdown(
    ...
    test('runOcrProcess success sets ocrCandidate and applyOcrCandidate updates bill', () async {
      ...
      when(() => mockRepository.getGroupMembers(groupId: any(named: 'groupId')))
          .thenAnswer((_) async => Right(sampleMembers));
      when(() => mockRepository.createBillWithPhotos(
            groupId: any(named: 'groupId'),
    ...
    test('loadBillDetail with empty billId loads members and auto-resolves creditor without calling getBillDetail', () async {
      when(() => mockRepository.getGroupMembers(groupId: any(named: 'groupId')))
          .thenAnswer((_) async => Right(sampleMembers));
      ...
      verifyNever(() => mockRepository.getBillDetail(billId: any(named: 'billId'), groupId: any(named: 'groupId')));
    });
  });
  ...
  group('BillDetailPage Widget Tests', () {
    testWidgets('renders all main bill components properly', (tester) async {
      when(() => mockRepository.getGroupMembers(groupId: any(named: 'groupId')))
          .thenAnswer((_) async => Right(sampleMembers));
      when(() => mockRepository.getBillDetail(billId: any(named: 'billId'), groupId: any(named: 'groupId')))
          .thenAnswer((_) async => Right(sampleBill));

      await tester.pumpWidget(
        ProviderScope(
          child: MaterialApp(
            home: BillDetailPage(initialBill: sampleBill),
          ),
        ),
      );
